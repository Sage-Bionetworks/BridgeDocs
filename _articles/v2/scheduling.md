...
|-------|------|-------------|
| eventId | Y | A string identifying this event. The event ID must be in a specific format based on the type of event (e.g. `study_burst:foo:02` or `custom:event1`). |
| timestamp | Y | The time this event occurred. This timestamp may be generated by the server for system events, or it may be submitted by a client, in which case it will be according to the client’s clock which may be skewed from the server or actual time. The `createdOn` field can help to identify such clients. |
| updateType | Y | The type of updates that are allowed for this event (described above). The update type is carried over to the specific event records for informational purposes. |
| answerValue | N | For some event types (specifically, answers to questions), this field will have a value. It is also used to record the iteration of a study burst event. |
| clientTimeZone | N | Events can be created on the server with a time zone name (e.g. "America/Los_Angeles") so that clients can display the event in the local time zone of the user. Note that this does not update if the user changes time zones, and is not required. |
| originEventId | N | If this event is a study burst event, generated from an originating event that triggers the study burst, the name of that originating event will be available in this field. Authoring tools may wish to relate these events in an editor. |
| studyBurstId | N | If this event is a study burst event, the identifier of the study burst (the identifier is also part of the `eventId` of this event. |
| periodFromOrigin | N | If this event is a study burst event, the delay as a period from the originating event’s timestamp at which this event occurs. |
| recordCount | N | In summary views of events, the number of records for this event (assuming it is mutable and has a history of changes, otherwise this value will be 1). |
| createdOn | Y | The time this event was created on server’s clock. This value will always be present after the even it created. |
