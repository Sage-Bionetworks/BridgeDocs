...
|CriteriaScheduleStrategy|The most flexible strategy, where each schedule is evaluated sequentially against a set of [Criteria](/model-browser.html?url=/rest-api/{{site.data.versions.java_sdk}}/rest-api/swagger.json#Criteria) and the first criteria that match a participant's request, that schedule is used to schedule activities for the participant. To assure a default schedule for participants who do not match any other criteria, you can include a last [ScheduleCriteria](/model-browser.html?url=/rest-api/{{site.data.versions.java_sdk}}/rest-api/swagger.json#ScheduleCriteria) object at the end of the list with no defined criteria.|
